...
def analyzeGlobalVariables(file):
    ...
    for line in file:
        ...
        if("}" in line):
            scopeCount = scopeCount - 1
        if(('int' in line) or ('bool' in line) or ('string' in line) or ('float' in line) or ('double' in line) or ('auto' in line) or ('char' in line) or (len(line) >= 4 and " " in line and '(' not in line and ')' not in line and "#include" not in line and len(line.split(" ")) >= 2 and ("namespace" not in line))):
